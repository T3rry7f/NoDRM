...
license_proxy_server=('10.211.55.4',8888) # widevine proxy server ipaddress
...
class PSSH:
    ...
    def parse(self,object,type=1):
        self.content=object
        if(type==1): # parse mpd file 
            try:
                matchObj = re.findall( r'pssh.+<', object, re.M|re.I)
                pssh_data= matchObj[0].split('>')[1].split('<')[0]
                self.data=base64.b64decode(pssh_data)
            except Exception as e:
               pass

        else: # parse mp4 init dataa
            try:
...
if __name__ == '__main__':
    ...
    if len(sys.argv)>1:
        ...
        pssh=PSSH()
        pssh.parse(data,type=1) 

        cdm=WidevineCDM('http://widevine-proxy.appspot.com/proxy')      #
        data=cdm.generateRequestData(pssh.data)
        cdm.getContentKey(data)
    else:
        print("Please input the mpd link")
